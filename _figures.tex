...
\int_new:N \l_largeurPlaceX_int
\int_new:N \l_hauteurPlaceY_int
\NewDocumentCommand{\rang}{D<>{3} D<>{2} >{\SplitList{,}} m >{\SplitList{,}} m >{\SplitList{,}} d[]}{
  \begin{tikzpicture}
    % Initialisation de la position horizontale et de la taille des places
    \int_set:Nn \l_rangPositionX_int {0}
    \int_set:Nn \l_largeurPlaceX_int {#1}
    \int_set:Nn \l_hauteurPlaceY_int {#2}
    % Première rangée
...
\NewDocumentCommand{\rangImpl}{m}{
  \int_add:Nn \l_rangPositionX_int {\l_largeurPlaceX_int}
  \texteCadre(\l_rangPositionX_int, 0) (\l_largeurPlaceX_int) (\l_hauteurPlaceY_int) {#1} %
}
\ExplSyntaxOff
