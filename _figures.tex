...
\NewDocumentCommand{\tikzCercle}{O{black} d() m o}{
  \IfValueTF{#4}{
...
% \tikzLabel (x, y) [texte] 
\NewDocumentCommand{\tikzLabel}{d() m}{
  \node at (#1) {#2};
}
% \tikzPointLabel (x, y) [texte], trace un point en plus 
\NewDocumentCommand{\tikzPointLabel}{d() m}{
  \filldraw (#1) circle (2pt) node[above] {#2};
}
% \tikzVecteur [couleur] (x, y) (lx, ly) {legende} [Position legende];  * = <->
\NewDocumentCommand{\tikzVecteur}{s O{black} d() d() m O{right}}{
  \coordinate (A) at (#3);
  \coordinate (B) at (#4);
