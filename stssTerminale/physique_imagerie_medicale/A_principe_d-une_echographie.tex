...
\question{
  ...
  $\Delta t_A$ correspond au temps mis par l'onde pour faire un aller-retour, il faut donc le diviser par 2 pour calculer la distance
  \begin{align*}
    d_A &= c \times \dfrac{\Delta t_A}{2} \\
  \end{align*}
}[3]

\question{
  Exprimer de même la distance $d_B$ entre la sonde et la seconde oreille,
  en fonction de la célérité des ultrasons $c$
  et du temps de détection des ultrasons réfléchis $\Delta t_B$.
}{
  C'est le même calcul, mais avec $\Delta t_B$ pour le temps
  \begin{align*}
    d_B &= c \times \dfrac{\Delta t_B}{2} \\
  \end{align*}
}[3]
...
\question{
  ...
  Le diamètre bipariétal vaut 
  \begin{align*}
    BIP &= d_B - d_A \\
    &= c \times \dfrac{\Delta t_B}{2} - c\times \dfrac{\Delta t_A}{2} \\
    &= c \times \left(\dfrac{\Delta t_B - \Delta t_A}{2}\right) \\
    &= \qty{1540}{\m\per\s} \times \left(\dfrac{(185 - 120)\times\qty{e-6}{\micro\s}}{2}\right) \\
    &= \qty{5.0e-2}{\m} \\
    &= \qty{50}{\milli\m}
  \end{align*}
}[5]
...
\question{
  ...
  Le diamètre bipariétal est compris entre les deux valeurs normales, $\qty{46}{\milli\m} < \qty{50}{\milli\m} < \qty{57}{\milli\m}$, donc on ne peut pas suspecter un retard de croissance pour le fœtus.
}[5]
