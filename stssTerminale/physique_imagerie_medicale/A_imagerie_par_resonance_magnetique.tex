...
%%%% objectifs
\begin{objectifs}
  \item Comprendre le principe de l'IRM et l'intérêt des produits de contrastes.
\end{objectifs}
...
\begin{doc}{Principe d'une IRM}
  \qrcodeCote{https://www.youtube.com/watch?v=49tgEcMA7kM}(-30pt)
...
\question{
  ...
  Les molécules complexes sont plus riches en électrons, qui vont « protéger » les noyaux d'hydrogènes du champs magnétique extérieur, ce qui entraine une aimantation plus faible.
}[4]

\question{
  Expliquer pourquoi les tissus osseux ne sont pas visible en IRM.
}{
  C'est parce que les tissus osseux contiennent peu de molécules d'eau, leur aimantation est donc faible et ils sont peu visible avec une IRM.
}[3]
...
\begin{doc}{Les produits de contraste pour l'IRM}
  ...
  % TODO-IMAGE : Mettre les images d'irm avec et sans produit de contraste
  %\vspace*{-16pt}
  %\begin{center}
  %  \chemfig{
  %    ^{-}OOC -[::0] !\lb
  %    N (!\ldb !\lb HOOC) -[::110] !\ldb !\ldb
  %    N (!\vide{-3, 0.8} Gd^{3+}) (-[3]-[2] COO^{-})
  %    -[::100] !\ldb !\ldb
  %    N (-[::120] !\lb COOH) -[::0] !\lh COO^{-}
  %  } \\[6pt]
  %  \important{3}
  %\end{center}
\end{doc}
...
\question{
  ...
  C'est le chélate \important{2}, il possède 4 fonctions amines autour du gadolinium.
}[3]


\question{
  Expliquer l'intérêt d'un produit de contraste.
}{
  Le produit de contraste permet d'améliorer la visibilité d'un organe précis sur une IRM, en augmentant temporairement son aimantation.
}[4]