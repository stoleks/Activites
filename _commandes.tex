...
\makeatletter % @ désactivé, permet de clarifier visuellement où se trouve les variables
...
\newcommand{\boiteRectangle}[2][]{
    ...
    \pgfmathsetlength{\@tailleBoite}{max(\@largeurBoite, (\@hauteurBoite + \@profondeurBoite))}
    \tikz \node [inner sep=2.5\pgflinewidth, minimum width=\@tailleBoite, minimum height=2\@hauteurBoite, #1] {#2};
}%
\makeatother
%%
...
\newdimen\@hauteurChiffre
\newdimen\@largeurChiffre
\newdimen\@profondeurChiffre
\newdimen\@tailleChiffre
\newdimen\@hauteurTexte
\newdimen\@largeurTexte
\newdimen\@hauteurMax
\newcommand{\boiteTitre}[3][]{
    \settoheight{\@hauteurChiffre}{#2}    % Hauteur du numéro
    \settowidth {\@largeurChiffre}{#2}    % Largeur du numéro
    \settodepth {\@profondeurChiffre}{#2} % Profondeur du numéro
    \settoheight{\@hauteurTexte}{#3}      % Hauteur du texte
    \settowidth {\@largeurTexte}{#3}      % Largeur du texte
    \pgfmathsetlength{\@hauteurMax}{max(\@hauteurTexte, \@hauteurChiffre)}
    \pgfmathsetlength{\@tailleChiffre}{max(\@largeurChiffre, (\@hauteurChiffre + \@profondeurChiffre))}
    %% Trace le numéro
    \tikz \node [inner sep=2.5\pgflinewidth, minimum width=\@tailleChiffre, minimum height=2\@hauteurMax, rounded corners = 2pt, fill = #1] {#2};
    %% Trace le titre
    \hspace*{-12pt}\noindent \tikz \node [minimum width = \@largeurTexte, minimum height = \@hauteurMax] {#3 };
    %% Trace la boite de fin
    \hspace*{-12pt}\noindent \tikz \fill [y = 2\@hauteurMax, rounded corners = 2pt, fill = #1] 
...
\newcommand{\implNomPrenom}{
  Nom : \lignePointillee[0.3\linewidth]
  Prénom : \lignePointillee[0.3\linewidth]
}
\newcommand{\nomPrenomClasse}{
  \setcounter{page}{1}
  \pasCorrection{ \vspace*{-24pt}
    \implNomPrenom Classe : \dotfill
  }
}
\newcommand{\nomPrenom}{
  \pasCorrection{ \vspace*{-24pt} \implNomPrenom }
}
...
\NewDocumentCommand{\qrcodeCote}{o +m D(){-16pt} O{1.5cm}}{
  \IfNoValueTF{#1} {
    \begin{wrapfigure}{r}{0.1\linewidth}
      \vspace*{#3}
      \qrcode[height = #4, color = couleurPrim]{#2}
    \end{wrapfigure}
  }{
...
%%%% atome ou isotope 
%#1: X, #2: Z, #3: A
\makeatletter% Permet de clarifier visuellement où se trouve les variables
\newdimen\@dimAtomNum
\newdimen\@dimMassNum
\newdimen\@dimExposant
\newcommand{\isotope}[3]{%
