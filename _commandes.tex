...
\makeatletter % @ désactivé, permet de clarifier visuellement où se trouvent les variables
...
\newtcolorbox{doc}[3][]{%
  ...
  titre avec compteur = {documentNum}{Document \arabic{documentNum} -- #2\strut \label{#3}},%
  #1
}
...
\pgfkeys{
  % définition de la famille de clefs
...
\NewDocumentEnvironment{TP}{m}{%
  \pgfkeys{/boiteActi, default, #1, compteur = TPNum, format = \mesure*\!\!TP}
  \begin{boiteActivite}
}{%
  \end{boiteActivite}
}
...
\newtcbox{\seance}[2][]{%
  boite cassable,%
  titre sans separation = 0.5mm,%
  titre avec compteur = {seanceNum}{Séance \arabic{seanceNum} \hfill (#2)},%
...
\newtcolorbox{boiteExtrait}[1][]{
  boite cassable, couleur fond = white, colframe = couleurSec-150,%
  overlay= {%
...
\NewDocumentCommand{\mesure}{s o}{%
  \questionSpeciale{#1}[#2]{\faFlask~\faMicroscope}
}
%
\NewDocumentCommand{\telechargement}{s o}{%
  \questionSpeciale{#1}[#2]{\faMobile*~\faFileDownload}
}
%
\NewDocumentCommand{\documentaire}{s o}{%
  \questionSpeciale{#1}[#2]{\faFile*~\faPen}
}
%
