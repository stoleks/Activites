...
%%%%
\begin{doc}{Mesure de concentration d'une solution colorée}
  \begin{importants}
...
% \correction{
%   Note : à partir d'une solution de concentration $c = \qty{0.2}{\mole\per\l}$, soit 
%   \begin{equation*}
%     c_m = \qty{118}{\g\per\mole}\times \qty{0.2}{\mole\per\l} = \qty{23.6}{\g\per\mole}
%   \end{equation*}
%   on peut diluer 25 fois pour arriver à une solution de concentration $c_m \approx \qty{0.95}{\g\per\l}$, et ensuite diluer par 5 à chaque fois.
%   Ou alors diluer par 125 pour avoir une solution moins dangereuse avec $c_m = \qty{0.19}{\g\per\l} $.
% }
...
\question{
  ...
  \begin{center}
    \begin{tblr}{c | X[c] | X[c] | X[c] | X[c] | X[c] }
...
%
\question{
  Sachant que le volume de la fiole jaugée est $V_1 = \qty{50}{\ml}$, donner le volume de la solution mère $V_0$ à prélever pour avoir un facteur de dilution $F = 5$.
}{
  On doit avoir un volume cinq fois plus faible, soit $V_0 = \qty{10}{\ml}$.
}[2]
...
\correction{Oui, on trouve une concentration $\qty{0,038}{\g/\litre} < c < \qty{0,0076}{\g/\litre}$.}

%
\question{
  Proposer une autre échelle de teinte pour améliorer la précision de la mesure (donner une liste de concentration).
}{
  On pourrait utiliser une échelle de teinte avec les concentrations suivantes : \numlist{0.038; 0.019; 0.0095; 0.00475} \unit{\g/\litre} ($F = 1.25$).
}[1]