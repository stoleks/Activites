%%%%
\teteSndAtom

%%%% titre
\titreTP{Fabriquer un atome}


%%%% Objectifs
\begin{objectifs}
  \item Apprendre la composition d'un atome.
  \item Comprendre la différence entre ion et atome.
\end{objectifs}

\begin{contexte}
  Au cours du \siecle{19}, la communauté scientifique considérait que l'atome était la plus petite « brique  » de la matière.
  Au début du \siecle{20}, deux expériences vont montrer que l'atome est composé de particules plus élémentaires :
  \begin{listePoints}
    \item en 1897, Thomson montre que l'on peut arracher des particules de charges négatives d'un atome ;
    \item en 1911, Rutherford montre que l'atome possède un noyau très petit devant la taille d'un atome, avec une charge positive.
  \end{listePoints}
  
  \problematique{
    Quelles entités composent les atomes ?
  }
\end{contexte}


%%%%
% \titreSection{L'atome}

\begin{doc}{Structure d'un atome}
  \begin{importants}
    Un atome est composé d'un \important{cortège électronique,} qui contient des \important{électrons chargés négativement,} et d'un \important{noyau,} qui contient des \important{nucléons.}    
    Il existe deux types de nucléons : les \important{protons chargés positivement} et les \important{neutrons qui sont neutres.}
  \end{importants}
\end{doc}

%%%%
\numeroQuestion
Légender cette représentation d'un atome en utilisant les mots proton, neutron, électron et nucléons.

\begin{center}
  \pasCorrection{\image{0.5}{images/atomes/atome}}
  \correction{\image{0.5}{images/atomes/atome_noyau}}
\end{center}


\qrcodeCote[2]{https://edurl.fr/P6W5vHoc}(-34pt)
%https://phet.colorado.edu/sims/html/build-an-atom/latest/build-an-atom_fr.html}

\telechargement Scanner le qrcode, puis cliquer sur « symbole » pour accéder à l'application qui va aider à répondre aux questions.

\question{
  Dans l'application le cadre « symbole  » indique l'élément chimique fabriqué.
  Quelle particule faut-il ajouter pour passer d'un élément chimique à un autre ?
}{
  Il faut ajouter des protons.
}[2]

\begin{doc}{Notation d'un élément chimique}[\label{doc:notation_element}]
  Pour distinguer les atomes on utilise la notation \isotope{X}{Z}{A}.
  \begin{importants}
    \begin{listePoints}
      \item \chemfig{X} est le symbole de l'atome considéré.
      \item $Z$ est le nombre de \texteTrou[0.3]{protons}, appelé \important{numéro atomique.}
      \item $A$ est le nombre de \texteTrou[0.3]{neutrons}, appelé \important{nombre de masse.}
    \end{listePoints}
  \end{importants}
\end{doc}

% \documentaire 
% Compléter le document~\ref{doc:notation_element}.

\numeroQuestion
\isotope{Na}{11}{23} : le sodium \chemfig{Na} possède \texteTrou{11} protons, \texteTrou{23} nucléons, \texteTrou{12} neutrons.


%%%%
% \titreSection{Les ions et les isotopes}

\question{
  Vérifier que la case « Neutralité/Ionisation » est cochée.
  Dans quel cas un élément chimique est un atome neutre ?
}{
  L'élément chimique est un atome s'il a autant d'électrons que de protons.
  Sinon il possède une charge électrique et c'est un ion.
}[2]

\begin{doc}{Les ions}
  \begin{importants}
    Les \important{ions} sont des éléments chimiques qui ont un électron en plus ou en moins.
  \end{importants}
\end{doc}

\question{
  En vous aidant de l'application, que signifie le « + » de \ionSodium ? Donner la composition de l'élément, c'est-à-dire son nombre de protons, neutrons et électrons.
}{
  Le « + » signifie qu'il y a une charge électrique positive autour de l'ion.
  Le \ionSodium possède le même nombre de protons et de neutrons que le sodium (11 protons et 12 neutrons), mais il n'a que 10 électrons.
}[3]

\question{
  Indiquer combien d'électrons l'ion chlorure \chlorure et l'ion cuivrique \chemfig{Cu^{2+}} ont perdu ou gagné par rapport à leur atome respectif.
}{
  L'ion chlorure a un électron supplémentaire par rapport à l'atome de chlore.
  L'ion cuivrique a deux électrons en moins par rapport à l'atome de cuivre
}[2]


%%%%
% \titreSection{Les isotopes}
\question{
  Vérifier que la case « Stabilité/Instabilité » est cochée.
  Deux atomes du même élément chimiques peuvent-ils avoir des noyaux stables avec une composition différente ?
}{
  Oui, il peuvent avoir un nombre de neutrons différents, comme l'hélium 3 \isotope{He}{2}{3} et 4 \isotope{He}{2}{4}.
}[2]

\begin{doc}{Les isotopes}
  \begin{importants}
    Deux éléments chimiques qui ont le même nombre de protons, mais un nombre de neutrons différents sont appelés des \important{isotopes.}
    \isotope{C}{6}{12} et \isotope{C}{6}{13} sont isotopes.
   \end{importants}
\end{doc}

\question{
  Quelle particule manque à l'élément \isotope{He}{2}{2} pour être stable ?
}{
  Il lui manque un ou deux neutrons.
}[1]
