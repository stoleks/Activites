...
\telechargement Télécharger l'application FizziQ en scannant le QR code du document~\ref{doc:chrono}.
...
\question{
  ...
  Non, la courbe ne passe pas à proximité de tous les points.
}[2]
...
\question{
  ...
  Oui la courbe passe à proximité de tous les points, et $a = \qty{4,85}{\m\per\s\squared}$
}[2]
...
\question{
  ...
  C'est un modèle linéaire qui décrit le mieux la vitesse verticale en fonction du temps, $a = \qty{9.8}{\m\per\s}$.
}[2]
...
\begin{doc}{Mouvement de la chute d'une balle dans un référentiel en mouvement}[\label{doc:chute_mouvement}]
  ...
  Finalement, il ne reste plus qu'à \important{répéter le protocole du document~\ref{doc:chrono},} pour repérer la position de la balle au cours du temps.
\end{doc}

\mesure Par groupe de 3, réaliser le protocole du document~\ref{doc:chute_mouvement}, puis sauvegarder le temps $T$, la position verticale $y$ et la vitesse verticale $V_y$.

\question{
  Tracer le graphique de $y(T)$. La trajectoire est-elle identique à celle de la chute libre ?
}{
  Non, mais on retrouve une trajectoire avec la même forme que dans le cas immobile.
}[1]
...
\question{
  ...
  Le meilleur modèle est le quadratique, on trouve $a = \qty{4.75}{\m\per\s\squared}$
}[2]

\question{
  Même question pour la vitesse $V_y$.
}{
  Le meilleur modèle est le modèle linéaire, avec $a = \qty{9.9}{\m\per\s}$.
}[2]
...
\question{
  ...
  Non, on retrouve les mêmes modèles, ce qui a changé c'est qu'il y a une vitesse horizontale en plus !
}[3]