...
\newpage
\vspace*{-40pt}
\begin{doc}{Exemple de trajectoire tracée par le programme}{doc:A3_exemple_trajectoire}
  \begin{center}  
...
%%
\question{
  Dans le programme python du document~\ref{doc:A3_code_python}, repérer les lignes de code permettant de calculer la position de la balle.
}{}[1]


\question{
  Dans le programme python du document~\ref{doc:A3_code_python}, repérer les lignes de code permettant de choisir la valeur de la vitesse $v_0$ ainsi que la valeur de l’angle $\alpha$.
}{}[1]

\question{
  D’après les documents~\ref{doc:A3_lancer_franc} et~\ref{doc:A3_code_python}, quelles sont les valeurs des coordonnées $(x ; y)$ que doit atteindre le ballon pour que le panier soit marqué ? Schématiser ce point sur le graphique du document~\ref{doc:A3_exemple_trajectoire}.
}{
  $(\num{4,70} ; \num{0,62})$
}[1]
...
\begin{doc}{Vecteur vitesse}{doc:A3_vitesse}
  \begin{importants}
