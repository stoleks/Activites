...
%%%% docs
\begin{doc}{Distance d'arrêt, de freinage et de réaction}[\label{doc:distances_arret}]
  \begin{center}
    \image{1}{images/mecanique/distance_arret}
  \end{center}
  \begin{importants}
    \begin{listePoints}
      \item La \important{distance de réaction $d_R$} est la distance que parcourt un véhicule entre le moment où la conductrice voit l'obstacle et quand elle commence à freiner.
      \item La \important{distance de freinage $d_F$} est la distance que parcourt un véhicule entre le moment où la conductrice commence à freiner et quand le véhicule est à l'arrêt.
      \item La \important{distance d'arrêt $d_A$} est simplement la somme de ces deux distances $d_A = d_R + d_F$.
    \end{listePoints}
  ...
  \begin{tblr}{
    ...
    & 11,1 & 22,2 & \correction{25} & \correction{30,6} & \correction{36,1} \\
    %
    $d_R$ (\unit{\m})
    & 11,1 & \correction{22,2} & 25,0 & \correction{30,6} & \correction{36,1} \\
    %
    $d_F$ sur sol sec (\unit{\m})
    & 10,3 & \correction{41,2} & 52,0 & 78,1 & 108,5 \\
    %
    ...
    & \correction{26,1} & \correction{82,1} & \correction{139,3} & 194,7 & \\
    %
  \end{tblr}
\end{doc}

\numeroQuestion
Compléter le tableau du document~\ref{doc:distances_arret}

\question{
  Expliquer pourquoi l'état du sol n'est pas indiqué pour la distance de réaction $d_R$.
}{
  Elle ne double pas, car elle dépend uniquement de la vitesse du véhicule et du temps de réaction d'une personne, temps qui est supposé constant ici.
}[2]

\question{
  Indiquer si la voiture va s'arrêter avant de percuter l'enfant, sur sol sec ou mouillé.
}{
  Sur sol sec, la voiture s'arrête au bout de \qty{21,4}{\m} et ne percute pas l'enfant.
  Sur sol mouillé en revanche, la voiture percute l'enfant, car elle s'arrête au bout de \qty{26,1}{\m}
}[2]
...
\question{
  ...
  Non, elle est multipliée par 4 quand on passe de \num{40} à \qty{80}{\km\per\hour}.
}[2]
...
\question{
  ...
  Oui, car si on trace $d_F (v^2)$ ou $v^2(d_F)$, on trouve que les deux grandeurs sont reliées par une droite passant par l'origine graphiquement.
}[2]
...
%%
\begin{doc}{Les facteurs intervenant dans les accidents de la route}
  \begin{listePoints}[2]
...
\begin{doc}{L'évolution du nombre de morts à cause des accidents de la route}
  \centering
  \image{0.78}{images/exterieures/donnees/evolution-nombre-de-morts-sur-les-routes}
\end{doc}
