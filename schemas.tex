...
\NewDocumentCommand{\emploiDuTemps}{O{}}{
  ...
  \foreach \h [count=\i] in \heures {
    \pgfmathsetlength{\edtY}{-\i*\edtHauteurCreneau}
    \node at (\edtX, \edtY) {\h};
...
\NewDocumentCommand{\evenement}{O{}}{
  ...
  \pgfmathsetlength{\edtY}{-\edtCreneau*\edtHauteurCreneau}
  % dessine le créneau
  \filldraw [fill = \edtCouleur, draw = \edtCouleur!75!black, line width = 2pt, rounded corners = 6pt]
    (\edtX, \edtY) rectangle (\edtX + \edtLargeurJour, \edtY - \edtDuree*\edtHauteurCreneau)
    node[pos=.5, text width = \edtLargeurJour, align = center] {
